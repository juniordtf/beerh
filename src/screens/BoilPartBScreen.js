...
class BoilPartBScreen extends Component {
  ...
  whenTimerIsDone = async () => {
    while (true) {
      if (
        ...
        this.playAlarmSound();
        Alert.alert('Tempo da 2ª fervura alcançado!');
        break;
      }
      await sleep(1000);
    }
  ...
  getHopName() {
    let currentRecipe = this.state.todaysRecipe;
    let hopName = currentRecipe.boil ? currentRecipe.boil[1].name : '';
    return hopName;
  }

  getHopQuantity() {
    let currentRecipe = this.state.todaysRecipe;
    let hopQuantity = currentRecipe.boil ? currentRecipe.boil[1].quantity : 1;
    return hopQuantity;
  }

  getHopUnit() {
    let currentRecipe = this.state.todaysRecipe;
    let hopUnit = currentRecipe.boil ? currentRecipe.boil[1].unit : 'g';
    return hopUnit;
  }
          ...
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              <Text style={styles.modalText}>
                O tempo da 2ª fervura ainda não foi atingido. Deseja realmente
                avançar?
              </Text>
              <View style={styles.rowContainer}>
                <TouchableHighlight
