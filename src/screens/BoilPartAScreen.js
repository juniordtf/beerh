...
class BoilPartAScreen extends Component {
  constructor(props) {
    super(props);
  ...
  whenTimerIsDone = async () => {
    while (true) {
      if (
        ...
        this.playAlarmSound();
        Alert.alert('Tempo da 1ª fervura alcançado!');
        break;
      }
      await sleep(1000);
    }
  ...
  getHopName() {
    let currentRecipe = this.state.todaysRecipe;
    let hopName = currentRecipe.boil ? currentRecipe.boil[0].name : '';
    return hopName;
  }

  getHopQuantity() {
    let currentRecipe = this.state.todaysRecipe;
    let hopQuantity = currentRecipe.boil ? currentRecipe.boil[0].quantity : 1;
    return hopQuantity;
  }

  getHopUnit() {
    let currentRecipe = this.state.todaysRecipe;
    let hopUnit = currentRecipe.boil ? currentRecipe.boil[0].unit : 'g';
    return hopUnit;
  }
          ...
          <View style={styles.centeredView}>
            <View style={styles.modalView}>
              <Text style={styles.modalText}>
                O tempo da 1ª fervura ainda não foi atingido. Deseja realmente
                avançar?
              </Text>
              <View style={styles.rowContainer}>
                <TouchableHighlight
...
export default BoilPartAScreen;
